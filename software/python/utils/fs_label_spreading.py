...
def cosine_kernel(X, y=None, gamma=0.1):
    cosine_dist = lambda u,v : np.arccos(np.dot(u,v))
    if y is not None:
        return spatial.distance.cdist(np.reshape(y, (-1,3)), X, cosine_dist)
    else:
        return spatial.distance.cdist(X, X, cosine_dist)
...
class BaseLabelPropagation(BaseEstimator, ClassifierMixin):
    ...
    def _get_kernel(self, X, y=None):
        if self.kernel == "rbf":
            ...
            if y is None:
                return cosine_kernel(X, X, gamma=self.gamma)
            else:
                return cosine_kernel(X, y, gamma=self.gamma)
