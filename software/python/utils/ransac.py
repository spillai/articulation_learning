...
class LineModel2(BaseModel):

    """Total least squares estimator for nD lines.

    The ``_params`` attribute contains the parameters in the following order::

        v, a: where v is the normalized vector, a the point on the line
    ...
    def estimate(self, data):
        ...
        if data.shape[0] > 1:  # well/over-determined
            # first principal component
            _, _, v = np.linalg.svd(data - X0)
    ...
    def residuals(self, data):
        ...
        v, a = self._params
        res = (a - data) - (v * np.inner((a - data), v).reshape((-1,1)))
        return np.linalg.norm(res, axis=1)

    def predict(self, data, params=None): 
        """Predict coordinates using the estimated model.
        ...
        if params is None:
            params = self._params
        v, a = self._params
        return v * np.inner((a - data), v).reshape((-1,1))
...
if __name__ == "__main__": 
    print 'Test ransac with LineModel'
    ...
    noise = [rnd.randint(0, 2 * (num_samples - 1)) for i in xrange(num_noise)]

    Y[rnd.sample(xrange(len(Y)), num_noise)] = noise
    ...
    # Run
    start_time = time()
    sac_model, inliers, residual = ransac(data, LineModel, min_samples=2, 
                                          residual_threshold=100.0, max_trials=100)
    end_time = time()
    mean_time = (end_time - start_time) / num_iterations

    # Summary
    if sac_model._params is not None:
        print ' Parameters '.center(40, '=')
        print sac_model._params
        print ' Residual '.center(40, '=')
        print residual
        print ' Time '.center(40, '=')
        print '%.1f msecs mean time spent per call' % (1000 * mean_time)
        print '%.1f msecs total time spent per call' % (1000 * (end_time - start_time))
        X = np.asarray([0, num_samples - 1])
        # Xe = np.array([X, np.zeros(len(X))]).T
        # Y = sac_model.predict(Xe)[:,1]
        Y = np.tan(np.pi/2 + sac_model._params[1]) * X + sac_model._params[0]
        plt.plot(X, Y, 'k-')
    else:
