...
namespace opencv_utils { 
    ...
    void convert_to_argb(const cv::Mat& img_mat, unsigned int* img) { 
        ...
        int idx_i = 0;
        for (int i=0; i<img_mat.rows; i++, imgp += img_mat.step, idx_i += img_mat.cols) { 
            for (int j=0; j<img_mat.cols; j++) { 
                img[idx_i + j] = 0;
                img[idx_i + j] |= (unsigned int)(imgp[3*j+2]);
                img[idx_i + j] |= (unsigned int)(imgp[3*j+1]) << 8;
                img[idx_i + j] |= (unsigned int)(imgp[3*j]) << 16;
            }
        }
        return;
    }
    ...
    void convert_to_mat(unsigned int* img, cv::Mat& img_mat) { 
        ...
        for (int i=0; i<img_mat.rows; i++, imgp += img_mat.step, idx_i += img_mat.cols) { 
            for (int j=0; j<img_mat.cols; j++) { 
                imgp[3*j+2] = ((img[idx_i + j]) & 0xff);
                imgp[3*j+1] = ((img[idx_i + j] >> 8) & 0xff);
                imgp[3*j] = ((img[idx_i + j] >> 16) & 0xff);
            }
        }
        return;
    }
...
find_enclosing_contour(std::vector<cv::Point>& pts, const cv::Size& size,
                       const int& erode_iterations) {

  // Build mask
  cv::Mat1b mask = cv::Mat1b::zeros(size);
  for (int j=0; j<pts.size(); j++)
    mask(pts[j].y, pts[j].x) = 255; 

  // Erode mask before finding contours
  if (erode_iterations)
    erode(mask, mask, cv::Mat(), cv::Point(-1,-1), erode_iterations);

  // Find contours
  std::vector<std::vector<cv::Point> > contours;
  findContours(mask, contours, cv::RETR_LIST, cv::CHAIN_APPROX_SIMPLE);
...
// ===================================================================================
// MSER with floating point support
...
// template <typename T> 
static double ChiSquaredDistance( float* x, float* y, float scale = 128.f ) // hack
{
  return scale * ((double)((x[0]-y[0])*(x[0]-y[0]))/(double)(x[0]+y[0]+1e-10)+
                  (double)((x[1]-y[1])*(x[1]-y[1]))/(double)(x[1]+y[1]+1e-10)+
                  (double)((x[2]-y[2])*(x[2]-y[2]))/(double)(x[2]+y[2]+1e-10));
}
...
// the preprocess to get the edge list with proper gaussian blur
static int preprocessMSER_32FC3( MSCRNode* node,
            MSCREdge* edge,
            double* total,
            CvMat* src,
            CvMat* mask,
            ...
            int edgeBlurSize )
{
  int srccpt = src->step-src->cols*3*sizeof(float);
  uchar* srcptr = src->data.ptr;
  uchar* lastptr = src->data.ptr + 3 *sizeof(float);
  double* dxptr = dx->data.db;
  float *pval = 0, *nval = 0;
    for ( int i = 0; i < src->rows; i++ )
    {
        for ( int j = 0; j < src->cols-1; j++ )
        {
          pval = (float*) srcptr, nval = (float*) lastptr;
          if (pval[0] != pval[0] || nval[0] != nval[0])
            *dxptr = 0 ;
          else
            *dxptr = ChiSquaredDistance( pval, nval );
            dxptr++;
            srcptr += 3 * sizeof(float);
            lastptr += 3 * sizeof(float);
        }
        srcptr += srccpt+3 * sizeof(float);
        lastptr += srccpt+3 * sizeof(float);
    }
    srcptr = src->data.ptr;
    lastptr = src->data.ptr + src->step;
    double* dyptr = dy->data.db;
    for ( int i = 0; i < src->rows-1; i++ )
    {
        for ( int j = 0; j < src->cols; j++ )
        {
          ...
          if (pval[0] != pval[0] || nval[0] != nval[0])
            ...
            *dyptr = ChiSquaredDistance( pval, nval );
            dyptr++;
            srcptr += 3 * sizeof(float);
            lastptr += 3 * sizeof(float);
        }
        srcptr += srccpt * sizeof(float);
        lastptr += srccpt * sizeof(float);
    }
    // get dx and dy and blur it
    if ( edgeBlurSize >= 1 )
    {
             ...
             MSERParams params )
{
    ...
    CvMat* dx = cvCreateMat( src->rows, src->cols-1, CV_64FC1 );
    CvMat* dy = cvCreateMat( src->rows-1, src->cols, CV_64FC1 );
    Ne = preprocessMSER_32FC3( map, edge, &emean, src, mask, dx, dy, Ne, params.edgeBlurSize );
    emean = emean / (double)Ne;
    std::sort(edge, edge + Ne, LessThanEdge());
    MSCREdge* edge_ub = edge+Ne;
    MSCREdge* edgeptr = edge;
             ...
             MSERParams params )
{
    ...
    CV_Assert(CV_MAT_TYPE(src->type) == CV_8UC1 || CV_MAT_TYPE(src->type) == CV_8UC3 ||
              CV_MAT_TYPE(src->type) == CV_32FC1 || CV_MAT_TYPE(src->type) == CV_32FC3);
    ...
    switch ( CV_MAT_TYPE(src->type) )
    {
        case CV_8UC1:
          extractMSER_8UC1( src, mask, contours, storage, params );
            break;
        case CV_8UC3:
          extractMSER_8UC3( src, mask, _dx, _dy, contours, storage, params );
            break;
      case CV_32FC3:
        extractMSER_32FC3( src, mask, _dx, _dy, contours, storage, params );
            break;
...
void MSER::processImage(const cv::Mat& image, const cv::Mat& mask) {
  cv::Mat der; 
  (*this)(image, dstcontours_, hierarchy_, der, mask);
}

std::vector<std::vector<Point> > MSER::getContours() {
  return dstcontours_;
}

std::vector<cv::Vec4i> MSER::getHierarchy() {
  return hierarchy_;
}

void MSER::operator()( const Mat& image, std::vector<std::vector<Point> >& dstcontours,
                       std::vector<cv::Vec4i>& hierarchy, 
                       cv::Mat& der, const Mat& mask ) const
{
  ...
  cv::Mat dx(image.rows, image.cols-1, CV_64FC1);
  cv::Mat dy(image.rows-1, image.cols, CV_64FC1);
  CvMat _dx = dx, _dy = dy;
  
  MemStorage storage(cvCreateMemStorage(0));
  Seq<CvSeq*> contours;
  extractMSER( &_image, pmask, &_dx, &_dy, &contours.seq, storage,
  ...
  cv::Rect r(0, 0, image.cols-1, image.rows-1);
  cv::Mat dx1(dx, r), dy1(dy, r);
  der = dx1 * 0.5 + dy1 * 0.5; 
  // cv::imshow("der", der);
...
void MserFeatureDetector::detectImpl( const Mat& image, std::vector<KeyPoint>& keypoints,
                                      cv::Mat& der, const Mat& mask ) const
{
    std::vector<std::vector<Point> > msers;

    std::vector<cv::Vec4i> hierarchy;
    (*this)(image, msers, hierarchy, der, mask);
