...
cv::Mat RichFeatureMatcher::MatchFeatures(int idx_i, int idx_j, vector<DMatch>* matches) {
  ...
  stringstream ss; ss << "imgpts1 has " << imgpts1.size()
                      << " points (descriptors " << descriptors_1.rows << ")" << endl;
  cout << ss.str();
  stringstream ss1; ss1 << "imgpts2 has " << imgpts2.size()
                        << " points (descriptors " << descriptors_2.rows << ")" << endl;
  cout << ss1.str();
  ...
  assert(matches->size() > 0);
    ...
    cv::Mat F = GetFundamentalMat(keypoints_1,keypoints_2,
