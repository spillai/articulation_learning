...
GenericModelVector MultiModelFactory::createModels(const articulation::track_msg_t& trackMsg) {
...
GenericModelVector MultiModelFactory::createModels(const articulation::model_msg_t& modelMsg) {
	GenericModelVector models;
	for(size_t i=0;i<factories.size();i++) {
            models.push_back( factories[i]->createModel(modelMsg) );
	}
...
GenericModelPtr MultiModelFactory::restoreModel(const articulation::model_msg_t& modelMsg) {
    for(size_t i=0;i<factories.size();i++) {
