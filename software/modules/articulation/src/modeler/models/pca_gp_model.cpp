...
#include <articulation/pca_gp_model.h>
...
void PCAGPModel::writeParamsToModel() {
	...
	setParam("downsample",downsample,articulation::model_param_msg_t::PARAM);
	...
	setParam("training_samples",training_samples,articulation::model_param_msg_t::PARAM);
}

btTransform PCAGPModel::pose(size_t index) {
	btTransform pose;
...
//		cout <<"choosing j="<<j<<endl;
		setParam(str(format("pose[%d]")%i),poseToTransform(model.track.pose[j]),articulation::model_param_msg_t::PARAM);
	}

}

bool PCAGPModel::fitModel(bool optimize) {
...
void PCAGPModel::buildGPs() {
	...
	for (size_t j = 0; j < training_samples; j++) {
		btVector3 diff = pose(j).getOrigin() - rigid_position;
		X_trans(j, 0) = diff.x();
		X_trans(j, 1) = diff.y();
		X_trans(j, 2) = diff.z();
	}
...
//	cout << endl;
	...
	prismatic_dir= btVector3(v0,v1,v2);
//	PRINT(prismatic_dir);
...
//	PRINT(prismatic_dir);
//	writeParamsToModel();
//	prismatic_dir = btVector3(1,-1,0);
//	prismatic_dir = btVector3(0.48308,-0.87558,0.00000);
//	prismatic_dir = btVector3(0.88215,0.46638,-0.06560);
//	prismatic_dir = btVector3(prismatic_dir.x(),prismatic_dir.y(),prismatic_dir.z());
...
//	PRINT(prismatic_dir);
	...
	for(size_t i=0;i<training_samples;i++) {
		...
		articulation::pose_msg_t p = transformToPose(pose(i));
		input[i][0] = predictConfiguration( p )[0];
		output[0][i] = p.pos[0];
		output[1][i] = p.pos[1];
		output[2][i] = p.pos[2];
		output[3][i] = p.orientation[0]; // .w;
		output[4][i] = p.orientation[1]; // .x;
		output[5][i] = p.orientation[2]; // .y;
		output[6][i] = p.orientation[3]; // .z;

		// output[3][i] = p.orientation[3]; // .w;
		// output[4][i] = p.orientation[0]; // .x;
		// output[5][i] = p.orientation[1]; // .y;
		// output[6][i] = p.orientation[2]; // .z;
...
V_Configuration PCAGPModel::predictConfiguration(articulation::pose_msg_t objpose) { 
	btVector3 diff = (positionToVector(objpose.pos) - rigid_position);
//	PRINT(diff);
//	PRINT(prismatic_dir);
...
articulation::pose_msg_t PCAGPModel::predictPose(V_Configuration q) {
  articulation::pose_msg_t result;
	...
	btQuaternion pred_rot(mean_rx,mean_ry,mean_rz,mean_rw);
	btVector3 pred_pos(mean_x,mean_y,mean_z);
	pred_rot.normalize();
	btTransform pred_pose(pred_rot,pred_pos);
	result = transformToPose(pred_pose);

//	btTransform t( rigid_orientation, rigid_position + prismatic_dir*q[0]);
