...
#include <articulation/hogman_wrapper.h>
...
void add_edge(GraphOptimizer3D* optimizer, int id1, int id2, btTransform pose,
...
PoseMap hogman_solver(KinematicGraphType &graph, PoseIndex &poseIndex,
	...
	// reference
	btTransform root = poseToTransform(observed.begin()->second);

	int n=1000;

	// add predicted markers
	for (PoseMap::iterator it = predictedEmpty.begin(); it != predictedEmpty.end(); it++) {
		btTransform m = poseToTransform(it->second);
	...
	for (KinematicGraphType::iterator it = graph.begin(); it != graph.end(); it++) {
		...
		articulation::pose_msg_t& pose_pred = it->second->model.track.pose_projected[idx];

		assert(!(isnan(pose_pred.pos[0]) || isnan(pose_pred.pos[1]) || isnan(pose_pred.pos[2]) ||
                         isnan(pose_pred.orientation[0]) || isnan(pose_pred.orientation[1]) || isnan(pose_pred.orientation[2])|| isnan(pose_pred.orientation[3])));
	...
	for (PoseMap::iterator it = observed.begin(); it != observed.end(); it++) {
		...
		btTransform m = poseToTransform(observed.begin()->second).inverseTimes(
	...
	for (PoseMap::iterator it = observed.begin(); it != observed.end(); it++) {
		// relative transformation between marker 0 and marker it
		btTransform m = btTransform::getIdentity();
	...
	for (PoseMap::iterator it = predictedEmpty.begin(); it
		...
		btTransform d = btTransform(RPY_to_MAT(p[3], p[4], p[5]), btVector3(
				p[0], p[1], p[2]));
	...
	Transformation3 t = v->transformation;
	Vector6 p = t.toVector();
	btTransform d = btTransform(RPY_to_MAT(p[3], p[4], p[5]), btVector3(p[0],
			p[1], p[2]));
