...
namespace AISNavigation{
  ...
  bool CholOptimizer<PG>::initialize(int rootNode){
    if (GraphOptimizer<PG>::verbose())
      cerr << "# init " << rootNode << endl;
    if (GraphOptimizer<PG>::vertex(rootNode)){
      _rootNode=rootNode;
      return true;
    }
    _rootNode=-1;
  ...
  int CholOptimizer<PG>::optimize(int iterations, bool online){
    ...
    for (Graph::VertexIDMap::const_iterator it=GraphOptimizer<PG>::vertices().begin(); it!=GraphOptimizer<PG>::vertices().end(); it++){
      vset.insert(it->second);
    }
    
    typename PG::Vertex* root=dynamic_cast<typename PG::Vertex*>(GraphOptimizer<PG>::vertex(_rootNode));
    if (! root)
      root=_MY_CAST_<typename PG::Vertex*>(GraphOptimizer<PG>::vertices().begin()->second);
    if (GraphOptimizer<PG>::verbose())
      cerr << "# root id " << root->id() << endl;
    bool initFromObservations = GraphOptimizer<PG>::_guessOnEdges;
    CholOptimizer<PG>::optimizeSubset(root, vset, iterations, 0., initFromObservations);
    return iterations;
  }
      ...
      int otherNode, typename PG::InformationType* otherCovariance)
  {
    ...
    if (initFromObservations){
      CholOptimizer<PG>::initializeActiveSubsetWithObservations(rootVertex);
      if (GraphOptimizer<PG>::verbose()){
        cerr << "iteration= " << -1 
          << "\t chi2= " << this->chi2() 
          << "\t time= " << 0.0
	...
	typename PG::Vertex* otherVertex=_MY_CAST_<typename PG::Vertex*>(GraphOptimizer<PG>::vertex(otherNode));
  ...
  typename PG::Edge* CholOptimizer<PG>::addEdge(typename PG::Vertex* from, typename PG::Vertex* to, const typename PG::TransformationType& mean, const typename PG::InformationType& information){
    Graph::EdgeSet eset1=GraphOptimizer<PG>::connectingEdges(from, to);
    Graph::EdgeSet eset2=GraphOptimizer<PG>::connectingEdges(to, from);
    Graph::EdgeSet eset;
    ...
    if (eset.empty()){
      ...
      if (GraphOptimizer<PG>::_guessOnEdges && to->edges().size()==1 && ! to->fixed()){
    ...
    if (0 && GraphOptimizer<PG>::verbose()) {
    ...
    GraphOptimizer<PG>::refineEdge(origEdge, newMean, newInfo);
    return origEdge;
  ...
  void CholOptimizer<PG>::initializeActiveSubsetWithObservations(typename PG::Vertex* root, double maxDistance){
    ...
    CholOptimizer<PG>::propagateAlongDijkstraTree(root, dv.adjacencyMap(), false, true); 
    for (typename std::set<typename PG::Edge*>::iterator it=_activeEdges.begin(); it!=_activeEdges.end(); it++){
      typename PG::Vertex* from=static_cast<typename PG::Vertex*>((*it)->from());
      typename PG::Vertex* to=static_cast<typename PG::Vertex*>((*it)->to());
      if (from->tempIndex()==-1)
