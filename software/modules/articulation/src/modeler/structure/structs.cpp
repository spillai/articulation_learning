...
KinematicParams::KinematicParams() {
  ...
  sigmax_position = 0.05;
  sigmax_orientation = 0.1;
  eval_every = 1;
...
void KinematicParams::LoadParams(BotParam* b_server, bool show) {
  sigma_position = 
      bot_param_get_double_or_fail(b_server,
                                   "kinematic_params.sigma_position");
  sigma_orientation = 
      bot_param_get_double_or_fail(b_server,
                                   "kinematic_params.sigma_orientation");
  sigmax_position = 
      bot_param_get_double_or_fail(b_server,
                                   "kinematic_params.sigmax_position");
  sigmax_orientation = 
      bot_param_get_double_or_fail(b_server,
                                   "kinematic_params.sigmax_orientation");
  eval_every = 
      bot_param_get_int_or_fail(b_server,
                                "kinematic_params.eval_every");
  eval_every_power = 
      bot_param_get_double_or_fail(b_server,
                                   "kinematic_params.eval_every_power");
  supress_similar = 
      bot_param_get_boolean_or_fail(b_server,
                                    "kinematic_params.supress_similar");
  reuse_model = 
      bot_param_get_boolean_or_fail(b_server,
                                    "kinematic_params.reuse_model");

  char* filter_models_str = 
      bot_param_get_str_or_fail(b_server,
                                "kinematic_params.filter_models");

  char* restricted_graphs_str = 
      bot_param_get_str_or_fail(b_server,
                                "kinematic_params.restricted_graphs");
  restricted_graphs = std::string(restricted_graphs_str);

  restrict_graphs = 
      bot_param_get_boolean_or_fail(b_server,
                                    "kinematic_params.restrict_graphs");
  reduce_dofs = 
      bot_param_get_boolean_or_fail(b_server,
                                    "kinematic_params.reduce_dofs");
  search_all_models = 
      bot_param_get_boolean_or_fail(b_server,
                                    "kinematic_params.search_all_models");

  char* full_eval_str = 
      bot_param_get_str_or_fail(b_server,
                                "kinematic_params.full_eval");
  full_eval = std::string(full_eval_str);
  // restricted_graphs = " "+restricted_graphs+" ";

  string filter_models(filter_models_str);
  factory.setFilter(filter_models);
}

void KinematicData::addPose(articulation::pose_msg_t pose1, articulation::pose_msg_t pose2, 
                            size_t id1, size_t id2, KinematicParams &params) {
    track_msg_t_ptr& track = trajectories[id1][id2];
	if (!track) {
            track = boost::make_shared<articulation::track_msg_t>();
	...
	btTransform transf1 = poseToTransform(pose1);
	btTransform transf2 = poseToTransform(pose2);
	btTransform diff = transf1.inverseTimes(transf2);

	for(size_t i=0;i<trajectories[id1][id2]->pose.size();i++) {
          btTransform diff_to_known;
	...
	poseIndex[id1][id2][ pose1.utime ] = track->pose.size();

        articulation::pose_msg_t p = transformToPose( diff);
        assert(pose1.utime == pose2.utime);
        p.utime = pose1.utime; // pick either, should be synchronized
        
	track->pose.push_back(p);
	track->channels[0].values.push_back( pose1.utime );
        track->channels[0].num_values = track->channels[0].values.size();
...
void KinematicData::updateModel(size_t id1, size_t id2,KinematicParams &params, bool optimize) {
    track_msg_t_ptr& track = trajectories[id1][id2];
    GenericModelPtr& model = models[id1][id2];

    articulation::model_msg_t model_track;
    model_track.track = *track;
    // model_track.header = track->header;
    setParamIfNotDefined(model_track.params, "sigma_position",
                         params.sigma_position, articulation::model_param_msg_t::PRIOR);
    setParamIfNotDefined(model_track.params, "sigma_orientation",
                         params.sigma_orientation, articulation::model_param_msg_t::PRIOR);
    ...
    for (size_t i = 0; i < models_new.size(); i++) {
        if (!models_new[i]->fitModel(optimize)) continue;
    ...
    if(models_sorted.size()==0) {
        ...
        std::cerr << "no models!! model["<<id1<<","<<id2<<"]: poses="<<track->pose.size() << std::endl;;
        model.reset();
        return;
    }
...
std::vector<double> KinematicData::intersectionOfStamps() {
    ...
    for(map<double, map<int, pose_msg_t_constptr> >::iterator it = stampedMarker.begin();
...
PoseMap KinematicData::getObservation(double timestamp) {
    ...
    for(std::map<int, pose_msg_t_constptr>::iterator it = stampedMarker[timestamp].begin();
        it != stampedMarker[timestamp].end();it++) {
        observedMarkers[it->first] = *(it->second);
    }
...
void KinematicGraph::evaluateSingleStamp(double timestamp,
    ...
    for(PoseMap::iterator i=predictedMarkers.begin();i!=predictedMarkers.end();i++) {
        ...
        articulation::pose_msg_t p = *data.stampedMarker[timestamp][i->first];
        p = i->second;
        // FLAG!
        // ObjectPoseConstPtr pptr = boost::make_shared<ObjectPose>(p);
        pose_msg_t_constptr pptr = boost::make_shared<articulation::pose_msg_t>(p);
    ...
    for(PoseMap::iterator it = observationMarkers.begin();
        ...
        btTransform p1 = poseToTransform(observationMarkers[it->first]);
        btTransform p2 = poseToTransform(predictedMarkers[it->first]);
...
void KinematicGraph::projectConfigurationSpace(int reducedDOFs,std::vector<double> stamps, KinematicParams &params,KinematicData &data) {
    ...
    for(KinematicGraph::iterator it=begin(); it != end();it++) {
        ...
        for(size_t timestamp_idx = 0;timestamp_idx < stamps.size();timestamp_idx++) {
            ...
            model->model.track.pose_projected[idx].utime = stamps[timestamp_idx];
        }
    }
    DOF = reducedDOFs;
}
